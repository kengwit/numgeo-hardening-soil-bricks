...
!
! DESCRIPTION:
!> Expose constitutive model implementations of numgeo to the Abaqus user subroutine UMAT.
!> The implementation is designed to be flexible and allows to easily add new constitutive
!> models. The Hardening Soil (Matsuoka-Nakai, Bricks) model has no optional integration
!> properties, so the wrapper simply forwards the standard UMAT arguments to the model.
!
!> The following models are currently exposed:
...
!
!>### History
!>* 11.04.2026, J. Machacek - Initial version
!>* 01.06.2026, J. Machacek - Reworked for the standalone Hardening Soil release
!>* 02.07.2026, J. Machacek - Switched to the Hardening-Soil-MN-Bricks model (small-strain
!                            stiffness extension): nprops 14->16, nstatev 5->73, calls
!                            hardening_soil_MN_bricks / optimize_hs_bricks_internal_constants.
!                            Model identifier now matched with an exact string comparison
!                            instead of index(model,...)>0: a substring match is unsafe once
!                            another model name is a prefix of this one (as is the case here,
!                            'Hardening-Soil-MN' is a prefix of 'Hardening-Soil-MN-Bricks').
!
!=~~=~~=~~=~~=~~=~~=~~=~~=~~=~~=~~=~~=~~=~~=~~=~~=~~=~~=~~=~~=~~=~~=~~=~~=~~=~~=~~=~~=~~=~~=~~=~~=~~=~=~
