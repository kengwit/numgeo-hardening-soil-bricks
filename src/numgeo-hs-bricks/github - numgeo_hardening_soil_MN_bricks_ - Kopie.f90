...
module material_hardening_soil_MN_bricks_
  ...
  subroutine hardening_soil_MN_bricks(stress,statev,dds_dde,dstrain,time,dtime,ntens,nstatev,props,nprops,istep,iinc)
    ...
    !
     ...
     if (any(ieee_is_nan(stress))) then
  ...
  !> and lateral earth pressure coefficient (K0nc). A globalised Newton method
  !> with an Armijo backtracking line search solves the nonlinear system.
  !> Positive alpha and Hpp are enforced through logarithmic parameters.
  !>
  !> The iteration continues until both relative residuals fall below the
  !> specified tolerance (1e-6). The Jacobian is recomputed at every accepted
  !> iterate by central differences in logarithmic parameter space. A bounded
  !> logarithmic step and a steepest-descent fallback safeguard the iteration.
  !
  !> Original version (OPTIM) by Leonardo José Cocco
  !
  !>### History
  !>* 22.12.2025, J. Machacek - Initial version (refactoring, modernizing)
  !>* 28.12.2025, J. Machacek - Full Newton-Raphson, relative errors, Cramer solve, remove common block
  ...
  !>* 23.08.2026, J. Machacek - Globalise Newton iteration with an Armijo backtracking line search
  !>                          - Use scaled residuals and logarithmic parameters for alpha and Hpp
  !>                          - Add bounded steps, steepest-descent fallback and pivoted 2x2 solve
  !
  subroutine optimize_hs_bricks_internal_constants(props, nprops, history_file)
    ...
    real(rk), parameter :: STRESS_MIN = 1.0e-4_rk
    real(rk), parameter :: MAX_LOG_STEP = log(10.0_rk)
    integer, parameter :: MAXITER = 200

    real(rk) :: jacobian(2,2)
    real(rk) :: delta_log_X(2)
    real(rk) :: residual(2), residual_trial(2)
    real(rk) :: scaled_residual(2), scaled_residual_trial(2)
    real(rk) :: relative_error(2)
    real(rk) :: FVEC(2), FVEC_trial(2)
    real(rk) :: X(2), X_trial(2), X_reference(2)
    real(rk) :: log_X(2), log_X_trial(2)
    real(rk) :: merit, merit_trial
    real(rk) :: error_measure
    real(rk) :: step_length
    real(rk) :: directional_derivative
    integer :: iiter, backtracks
    integer :: history_unit, history_iostat
    character(len=512) :: history_iomsg
    logical :: write_history, valid, jacobian_valid
    logical :: solved, accepted, converged, newton_direction

    call mat%collect(props, nprops)

    ! Logarithmic, dimensionless variables guarantee alpha > 0 and Hpp > 0 and
    ! avoid the large difference in magnitude between both calibration parameters.
    X_reference = [1.0_rk, mat%Eoed]
    log_X = [log(1.20_rk), log(2.0_rk)]
    call calibration_parameters_from_log(log_X, X_reference, X, valid)
    if (.not. valid) then
      error stop 'optimize_hs_bricks_internal_constants: invalid initial parameter vector'
    end if

    call evaluate_calibration_state(mat, X, FVEC, residual, scaled_residual, merit, STRESS_MIN, valid)
    if (.not. valid) then
      error stop 'optimize_hs_bricks_internal_constants: invalid objective at initial parameter vector'
    end if

    relative_error = abs(scaled_residual)
    error_measure = maxval(relative_error)
    converged = error_measure <= TOL

    history_unit = 0
    write_history = present(history_file)
    if (write_history) then
      if (len_trim(history_file) == 0) then
        error stop 'optimize_hs_bricks_internal_constants: history_file must not be empty'
      end if

      open(newunit=history_unit, file=trim(history_file), status='replace', action='write', &
           form='formatted', iostat=history_iostat, iomsg=history_iomsg)
      if (history_iostat /= 0) then
        write(*,'(a)') 'optimize_hs_bricks_internal_constants: could not open convergence-history file'
        write(*,'(a)') '  file: '//trim(history_file)
        write(*,'(a)') '  reason: '//trim(history_iomsg)
        error stop
      end if
      ...
      call write_calibration_history_record(history_unit, 0, residual, relative_error, error_measure, &
                                            TOL, X, merit, 0.0_rk, 0)
    end if

    newton_loop: do iiter = 1, MAXITER
      if (converged) exit newton_loop

      call calibration_jacobian_log(mat, log_X, X_reference, jacobian, STRESS_MIN, jacobian_valid)
      if (.not. jacobian_valid) then
        call close_calibration_history(write_history, history_unit)
        error stop 'optimize_hs_bricks_internal_constants: failed to evaluate a finite Jacobian'
      end if

      call solve_calibration_system(jacobian, scaled_residual, delta_log_X, solved)
      newton_direction = solved

      if (solved) then
        directional_derivative = -dot_product(scaled_residual, matmul(jacobian, delta_log_X))
        newton_direction = ieee_is_finite(directional_derivative) .and. &
                           directional_derivative < 0.0_rk
      end if

      if (.not. newton_direction) then
        delta_log_X = matmul(transpose(jacobian), scaled_residual)
      end if

      call limit_calibration_direction(delta_log_X, MAX_LOG_STEP, valid)
      if (.not. valid) then
        call close_calibration_history(write_history, history_unit)
        error stop 'optimize_hs_bricks_internal_constants: no usable search direction'
      end if

      directional_derivative = -dot_product(scaled_residual, matmul(jacobian, delta_log_X))
      if (.not. ieee_is_finite(directional_derivative) .or. directional_derivative >= 0.0_rk) then
        call close_calibration_history(write_history, history_unit)
        error stop 'optimize_hs_bricks_internal_constants: search direction is not a descent direction'
      end if

      call calibration_line_search(mat, log_X, X_reference, delta_log_X, directional_derivative, &
                                   merit, STRESS_MIN, log_X_trial, X_trial, FVEC_trial, &
                                   residual_trial, scaled_residual_trial, merit_trial, step_length, &
                                   backtracks, accepted)

      ! If a valid Newton direction cannot be globalised, retry once with the
      ! negative gradient of the scaled least-squares merit function.
      if (.not. accepted .and. newton_direction) then
        delta_log_X = matmul(transpose(jacobian), scaled_residual)
        call limit_calibration_direction(delta_log_X, MAX_LOG_STEP, valid)

        if (valid) then
          directional_derivative = -dot_product(scaled_residual, matmul(jacobian, delta_log_X))
          valid = ieee_is_finite(directional_derivative) .and. directional_derivative < 0.0_rk
        end if

        if (valid) then
          call calibration_line_search(mat, log_X, X_reference, delta_log_X, &
                                       directional_derivative, merit, STRESS_MIN, log_X_trial, &
                                       X_trial, FVEC_trial, residual_trial, scaled_residual_trial, &
                                       merit_trial, step_length, backtracks, accepted)
        end if
      end if

      if (.not. accepted) then
        call close_calibration_history(write_history, history_unit)
        write(*,'(a,i0)') 'optimize_hs_bricks_internal_constants: line search failed at iteration ', iiter
        write(*,'(a,es16.8)') '  current maximum relative residual = ', error_measure
        write(*,'(a,2es16.8)') '  current alpha, Hpp = ', X(1), X(2)
        error stop
      end if
      ...
      scaled_residual = scaled_residual_trial
      merit = merit_trial
      ...
      if (write_history) then
        call write_calibration_history_record(history_unit, iiter, residual, relative_error, &
                                              error_measure, TOL, X, merit, step_length, backtracks)
      end if
    end do newton_loop
    ...
    if (.not. converged) then
      write(*,'(a,i0)') 'optimize_hs_bricks_internal_constants: no convergence after ', MAXITER
      write(*,'(a,es16.8)') '  maximum relative residual = ', error_measure
      write(*,'(a,2es16.8)') '  final alpha, Hpp = ', X(1), X(2)
      error stop
    end if
  ...
  !> date: 24.08.20226
  !
  !> Convert logarithmic dimensionless calibration variables to alpha and Hpp.
  !
  !>### History
  !>* 24.08.20226, J. Machacek - Initial version
  !
  subroutine calibration_parameters_from_log(log_X, X_reference, X, valid)
    use, intrinsic :: ieee_arithmetic, only: ieee_is_finite
    implicit none

    real(rk), intent(in) :: log_X(2)
    real(rk), intent(in) :: X_reference(2)
    real(rk), intent(out) :: X(2)
    logical, intent(out) :: valid

    real(rk) :: upper_log(2), lower_log(2)

    X = 0.0_rk
    valid = all(ieee_is_finite(log_X)) .and. all(X_reference > 0.0_rk) .and. &
            all(ieee_is_finite(X_reference))
    if (.not. valid) return

    upper_log = log(huge(1.0_rk)) - log(X_reference)
    lower_log = log(tiny(1.0_rk)) - log(X_reference)
    valid = all(log_X < upper_log) .and. all(log_X > lower_log)
    if (.not. valid) return

    X = X_reference * exp(log_X)
    valid = all(ieee_is_finite(X)) .and. all(X > 0.0_rk)

  end subroutine calibration_parameters_from_log
  ...
  !> date: 24.08.20226
  !
  !> Evaluate residuals and the scaled least-squares merit function.
  !
  !>### History
  !>* 24.08.20226, J. Machacek - Initial version
  !
  subroutine evaluate_calibration_state(mat, X, FVEC, residual, scaled_residual, merit, STRESS_MIN, valid)
    use, intrinsic :: ieee_arithmetic, only: ieee_is_finite
    implicit none
    ...
    real(rk), intent(out) :: residual(2)
    real(rk), intent(out) :: scaled_residual(2)
    real(rk), intent(out) :: merit
    ...
    FVEC = 0.0_rk
    residual = 0.0_rk
    scaled_residual = 0.0_rk
    merit = huge(1.0_rk)

    target = [mat%Eoed, mat%K0nc]
    valid = all(X > 0.0_rk) .and. all(ieee_is_finite(X)) .and. &
            all(target > 0.0_rk) .and. all(ieee_is_finite(target))
    if (.not. valid) return

    call objective_function(mat, X, FVEC, STRESS_MIN)
    valid = all(ieee_is_finite(FVEC))
    if (.not. valid) return

    residual = target - FVEC
    scaled_residual = residual / target
    merit = 0.5_rk * dot_product(scaled_residual, scaled_residual)
    valid = all(ieee_is_finite(residual)) .and. all(ieee_is_finite(scaled_residual)) .and. &
            ieee_is_finite(merit)

  end subroutine evaluate_calibration_state
  ...
  !> date: 24.08.20226
  !
  !> Compute the Jacobian of the scaled response with respect to logarithmic parameters.
  !
  !>### History
  !>* 24.08.20226, J. Machacek - Initial version
  !
  subroutine calibration_jacobian_log(mat, log_X, X_reference, jacobian, STRESS_MIN, valid)
    use, intrinsic :: ieee_arithmetic, only: ieee_is_finite
    implicit none
    ...
    real(rk), intent(in) :: log_X(2)
    real(rk), intent(in) :: X_reference(2)
    real(rk), intent(out) :: jacobian(2,2)
    real(rk), intent(in) :: STRESS_MIN
    logical, intent(out) :: valid

    real(rk), parameter :: DIFFERENCE_INTERVAL = 1.0e-2_rk
    real(rk) :: log_X_perturbed(2)
    real(rk) :: X_perturbed(2)
    real(rk) :: FVEC_minus(2), FVEC_plus(2)
    real(rk) :: target(2)
    real(rk) :: dummy_residual(2), dummy_scaled_residual(2), dummy_merit
    integer(ik) :: i_var
    logical :: minus_valid, plus_valid

    jacobian = 0.0_rk
    target = [mat%Eoed, mat%K0nc]
    valid = all(target > 0.0_rk) .and. all(ieee_is_finite(target))
    if (.not. valid) return

    do i_var = 1, 2
      log_X_perturbed = log_X
      log_X_perturbed(i_var) = log_X(i_var) - 0.5_rk * DIFFERENCE_INTERVAL
      call calibration_parameters_from_log(log_X_perturbed, X_reference, X_perturbed, minus_valid)
      if (minus_valid) then
        call evaluate_calibration_state(mat, X_perturbed, FVEC_minus, dummy_residual, &
                                        dummy_scaled_residual, dummy_merit, STRESS_MIN, minus_valid)
      end if

      log_X_perturbed = log_X
      log_X_perturbed(i_var) = log_X(i_var) + 0.5_rk * DIFFERENCE_INTERVAL
      call calibration_parameters_from_log(log_X_perturbed, X_reference, X_perturbed, plus_valid)
      if (plus_valid) then
        call evaluate_calibration_state(mat, X_perturbed, FVEC_plus, dummy_residual, &
                                        dummy_scaled_residual, dummy_merit, STRESS_MIN, plus_valid)
      end if
      ...
      jacobian(:,i_var) = (FVEC_plus / target - FVEC_minus / target) / DIFFERENCE_INTERVAL
    end do

    valid = all(ieee_is_finite(jacobian))

  end subroutine calibration_jacobian_log
  ...
  !> date: 24.08.20226
  !
  !> Solve a two-by-two system using Gaussian elimination with partial pivoting.
  !
  !>### History
  !>* 24.08.20226, J. Machacek - Initial version
  !
  subroutine solve_calibration_system(A, b, x, solved)
    use, intrinsic :: ieee_arithmetic, only: ieee_is_finite
    implicit none

    real(rk), intent(in) :: A(2,2)
    real(rk), intent(in) :: b(2)
    real(rk), intent(out) :: x(2)
    logical, intent(out) :: solved

    real(rk) :: work_A(2,2), work_b(2)
    real(rk) :: row_A(2), row_b
    real(rk) :: factor, matrix_scale, pivot_tolerance

    x = 0.0_rk
    solved = all(ieee_is_finite(A)) .and. all(ieee_is_finite(b))
    if (.not. solved) return

    work_A = A
    work_b = b
    matrix_scale = maxval(abs(work_A))
    if (matrix_scale <= tiny(1.0_rk)) then
      solved = .false.
      return
    end if

    pivot_tolerance = 100.0_rk * epsilon(1.0_rk) * matrix_scale

    if (abs(work_A(2,1)) > abs(work_A(1,1))) then
      row_A = work_A(1,:)
      work_A(1,:) = work_A(2,:)
      work_A(2,:) = row_A
      row_b = work_b(1)
    ...
    if (abs(work_A(1,1)) <= pivot_tolerance) then
      solved = .false.
      return
    end if

    factor = work_A(2,1) / work_A(1,1)
    work_A(2,2) = work_A(2,2) - factor * work_A(1,2)
    work_b(2) = work_b(2) - factor * work_b(1)

    if (abs(work_A(2,2)) <= pivot_tolerance) then
      solved = .false.
      return
    end if

    x(2) = work_b(2) / work_A(2,2)
    x(1) = (work_b(1) - work_A(1,2) * x(2)) / work_A(1,1)
    solved = all(ieee_is_finite(x))

  end subroutine solve_calibration_system
  ...
  !> date: 24.08.20226
  !
  !> Limit the infinity norm of a logarithmic search direction.
  !
  !>### History
  !>* 24.08.20226, J. Machacek - Initial version
  !
  subroutine limit_calibration_direction(direction, maximum_norm, valid)
    use, intrinsic :: ieee_arithmetic, only: ieee_is_finite
    implicit none

    real(rk), intent(inout) :: direction(2)
    real(rk), intent(in) :: maximum_norm
    logical, intent(out) :: valid

    real(rk) :: direction_norm

    valid = all(ieee_is_finite(direction)) .and. ieee_is_finite(maximum_norm) .and. &
            maximum_norm > 0.0_rk
    if (.not. valid) return

    direction_norm = maxval(abs(direction))
    valid = direction_norm > sqrt(tiny(1.0_rk))
    if (.not. valid) return

    if (direction_norm > maximum_norm) then
      direction = direction * (maximum_norm / direction_norm)
    end if

  end subroutine limit_calibration_direction
  ...
  !> date: 24.08.20226
  !
  !> Armijo backtracking line search for one calibration direction.
  !
  !>### History
  !>* 24.08.20226, J. Machacek - Initial version
  !
  subroutine calibration_line_search(mat, log_X, X_reference, direction, directional_derivative, &
                                     merit, STRESS_MIN, log_X_trial, X_trial, FVEC_trial, &
                                     residual_trial, scaled_residual_trial, merit_trial, step_length, &
                                     backtracks, accepted)
    use, intrinsic :: ieee_arithmetic, only: ieee_is_finite
    ...
    real(rk), intent(in) :: directional_derivative
    real(rk), intent(in) :: merit
    real(rk), intent(in) :: STRESS_MIN
    real(rk), intent(out) :: log_X_trial(2)
    real(rk), intent(out) :: X_trial(2)
    real(rk), intent(out) :: FVEC_trial(2)
    real(rk), intent(out) :: residual_trial(2)
    real(rk), intent(out) :: scaled_residual_trial(2)
    real(rk), intent(out) :: merit_trial
    real(rk), intent(out) :: step_length
    integer, intent(out) :: backtracks
    logical, intent(out) :: accepted

    real(rk), parameter :: ARMIJO_COEFFICIENT = 1.0e-4_rk
    real(rk), parameter :: BACKTRACK_FACTOR = 0.5_rk
    real(rk), parameter :: MIN_STEP_LENGTH = 2.0_rk**(-40)
    integer, parameter :: MAX_BACKTRACKS = 40
    ...
    X_trial = 0.0_rk
    FVEC_trial = 0.0_rk
    residual_trial = 0.0_rk
    scaled_residual_trial = 0.0_rk
    merit_trial = huge(1.0_rk)
    step_length = 1.0_rk
    backtracks = 0
    accepted = .false.

    if (.not. all(ieee_is_finite(direction)) .or. &
        .not. ieee_is_finite(directional_derivative) .or. directional_derivative >= 0.0_rk) return

    do backtracks = 0, MAX_BACKTRACKS
      log_X_trial = log_X + step_length * direction
      call calibration_parameters_from_log(log_X_trial, X_reference, X_trial, valid)

      if (valid) then
        call evaluate_calibration_state(mat, X_trial, FVEC_trial, residual_trial, &
                                        scaled_residual_trial, merit_trial, STRESS_MIN, valid)
      end if

      if (valid) then
        if (merit_trial <= merit + ARMIJO_COEFFICIENT * step_length * directional_derivative) then
          accepted = .true.
          return
        end if
      end if

      step_length = BACKTRACK_FACTOR * step_length
      if (step_length < MIN_STEP_LENGTH) return
    end do

  end subroutine calibration_line_search
  ...
  !> date: 24.08.20226
  !
  !> Write one accepted calibration iterate to the CSV history file.
  !
  !>### History
  !>* 24.08.20226, J. Machacek - Initial version
  !
  subroutine write_calibration_history_record(history_unit, iteration, residual, relative_error, error_measure, &
                                              tolerance, X, merit, step_length, backtracks)
    implicit none

    integer, intent(in) :: history_unit
    integer, intent(in) :: iteration
    real(rk), intent(in) :: residual(2)
    real(rk), intent(in) :: relative_error(2)
    real(rk), intent(in) :: error_measure
    real(rk), intent(in) :: tolerance
    real(rk), intent(in) :: X(2)
    real(rk), intent(in) :: merit
    real(rk), intent(in) :: step_length
    integer, intent(in) :: backtracks

    integer :: write_iostat
    character(len=512) :: write_iomsg

    write(history_unit,'(i0,10(",",es26.16e3),",",i0)',iostat=write_iostat, &
          iomsg=write_iomsg) iteration, residual(1), residual(2), relative_error(1), &
          relative_error(2), error_measure, tolerance, X(1), X(2), merit, step_length, &
          backtracks
    if (write_iostat /= 0) then
      write(*,'(a)') 'optimize_hs_bricks_internal_constants: failed to write convergence history'
      write(*,'(a)') '  reason: '//trim(write_iomsg)
      error stop
    end if

    flush(history_unit, iostat=write_iostat, iomsg=write_iomsg)
    if (write_iostat /= 0) then
      write(*,'(a)') 'optimize_hs_bricks_internal_constants: failed to flush convergence history'
      write(*,'(a)') '  reason: '//trim(write_iomsg)
      error stop
    end if

  end subroutine write_calibration_history_record
  ...
  !> date: 24.08.20226
  !
  !> Close the optional calibration-history stream.
  !
  !>### History
  !>* 24.08.20226, J. Machacek - Initial version
  !
  subroutine close_calibration_history(write_history, history_unit)
    implicit none

    logical, intent(in) :: write_history
    integer, intent(in) :: history_unit

    integer :: close_iostat
    character(len=512) :: close_iomsg

    if (.not. write_history) return

    close(history_unit, iostat=close_iostat, iomsg=close_iomsg)
    if (close_iostat /= 0) then
      write(*,'(a)') 'optimize_hs_bricks_internal_constants: failed to close convergence history'
      write(*,'(a)') '  reason: '//trim(close_iomsg)
      error stop
    end if

  end subroutine close_calibration_history
  ...
  subroutine objective_function(mat0,X,FVEC,STRESS_MIN)
    use compatibility_numgeo_, only: cartesian_to_principal_stress_analytical, roscoe_pq_voigt, invariants_J2_J3
    implicit none
